...
4. Ensure `www.mathewhartley.com` remains proxied through Cloudflare so the Worker route can run in front of the existing S3 origin. The route wildcard also invokes the Worker for guestbook-prefixed siblings, but its exact path check makes those requests return no-store 404; it never proxies/fetches them. The Worker does not call `fetch()` for API requests or access the Cache API. Every Worker response, including errors, has `Cache-Control: no-store`; verify no Cache Rule or Workers Cache setting overrides it for this API route.
...
6. Deploy the Worker code/route with `npx wrangler deploy`. This requires a Cloudflare API token with Workers and route permissions, plus `CLOUDFLARE_ACCOUNT_ID`; credentials are not stored here. Wrangler config routes `www.mathewhartley.com/api/guestbook*` on the `mathewhartley.com` zone. The Worker strictly handles only the exact endpoint and optional trailing slash; guestbook-prefixed sibling paths return a no-store 404 without fetching the S3 origin.

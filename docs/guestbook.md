...
4. Set `TURNSTILE_SECRET_KEY` as a Worker secret: `npx wrangler secret put TURNSTILE_SECRET_KEY`.
5. Confirm `GUESTBOOK_ORIGIN` is exactly `https://www.mathewhartley.com`. The two rate-limit bindings and their limits are declared in `wrangler.toml`. Cloudflare's Worker rate-limit bindings are best-effort and scoped to an edge location, not a globally coordinated per-IP limit.
6. Before deploying, replace `YOUR_TURNSTILE_SITE_KEY` in `src/pages/index.astro` with the real widget site key. This placeholder blocks successful submissions until configured. Create a Turnstile widget restricted to the intended hostname(s); the Worker checks the returned hostname. Do not deploy the guestbook as live until the real site key and secret are configured.
7. Ensure `www.mathewhartley.com` remains proxied through Cloudflare so the Worker route can run in front of the existing S3 origin. The route is narrow: the Worker returns a no-store 404 for any other path and does not proxy/fetch those paths. The Worker does not call `fetch()` for API requests or access the Cache API. Every Worker response, including errors, has `Cache-Control: no-store`; ensure no Cache Rule or Workers Cache setting overrides it for this API route.
8. Deploy the Worker manually with `npx wrangler deploy`. This requires a Cloudflare API token with permission to deploy Workers and manage routes, and the account ID for the account hosting the `mathewhartley.com` zone. Set `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` in the shell/environment before deploying; no credentials are stored in this repository. Wrangler's `[[routes]]` entry uses the route pattern `www.mathewhartley.com/api/guestbook*` and `zone_name = "mathewhartley.com"`; the zone name is the apex zone even though the route matches the `www` hostname.
